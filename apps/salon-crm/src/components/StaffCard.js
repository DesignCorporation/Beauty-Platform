...
export const StaffCard = ({ staff, selected, onClick, disabled = false, className = '' }) => {
    ...
    return (_jsx("div", { className: `flex-shrink-0 ${className}`, children: _jsxs("button", { onClick: onClick, disabled: disabled, title: `${staff.name}\n${staff.role}`, className: `
        ...
        `, onMouseEnter: (e) => {
                ...
                if (!selected) {
                    ...
                    if (img) {
                        ...
                        boxShadow: selected
                            ...
                            : '0 2px 8px rgba(0,0,0,0.1)'
                    }, children: [staff.avatarUrl ? (_jsx("img", { src: staff.avatarUrl, alt: staff.name, className: "rounded-full object-cover w-full h-full transition-all duration-300", style: {
                                ...
                                border: 'none'
                            }, children: initials })), isActive && (_jsx("div", { className: "absolute -top-1 -right-1 w-3 h-3 bg-green-400 rounded-full border-2 border-white" }))] }), _jsxs("div", { className: "mt-2 text-center", children: [_jsx("div", { className: `text-sm font-medium transition-colors ${selected ? 'text-blue-600' : 'text-gray-700'}`, children: staff.firstName || staff.name.split(' ')[0] }), _jsx("div", { className: "text-xs text-gray-500", children: staff.role === 'STAFF_MEMBER' ? 'Мастер' : staff.role })] }), _jsxs("div", { className: "absolute bottom-full left-1/2 transform -translate-x-1/2 mb-3 px-4 py-3 rounded-xl shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none whitespace-nowrap z-50", style: {
                        backgroundColor: getStaffColorValue(),
                        boxShadow: `0 8px 25px ${getStaffColorValue()}40`
                    }, children: [_jsx("div", { className: "font-semibold text-white text-sm", children: staff.name }), _jsx("div", { className: "text-white opacity-90 text-xs mt-0.5", children: staff.role === 'STAFF_MEMBER' ? 'Мастер' : staff.role }), _jsx("div", { className: "text-white opacity-80 text-xs", children: isActive ? '🟢 Активен' : '🔴 Не активен' })] })] }) }));
};
